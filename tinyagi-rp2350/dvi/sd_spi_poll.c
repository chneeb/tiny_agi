...
bool spi_transfer(spi_t *spi_p, const uint8_t *tx, uint8_t *rx, size_t length) {
    ...
    /* tinyagi DVI PATCH: poll the DMA hardware BUSY bit instead of waiting on
     * the completion semaphore via __wfe. On the dual-core DVI build the
     * cross-core WFE/event wakeup from the SD DMA-completion IRQ is
     * intermittently lost under core1 DVI-DMA load, so the semaphore wait
     * stalled up to the 1 s timeout -> failed read -> hang (worked around by a
     * keep-alive timer). dma_channel_is_busy() reads hardware directly and is
     * immune to the event race, so SD reads always complete promptly with no
     * timer dependency. (This vendored copy replaces the submodule's spi.c for
     * the DVI target only; picocalc/restouch use the stock file.) */
    absolute_time_t _sd_deadline = make_timeout_time_ms(1000);
    while (dma_channel_is_busy(spi_p->rx_dma)) {
        if (time_reached(_sd_deadline)) {
            DBG_PRINTF("DMA poll timed out in %s\n", __FUNCTION__);
            return false;
        }
        tight_loop_contents();
    }
    dma_channel_wait_for_finish_blocking(spi_p->tx_dma);  /* tx completes with rx */

    /* The DMA-completion IRQ still fires and releases the sem; drain it so the
     * sem state stays consistent (and debug-build asserts below hold). */
    sem_try_acquire(&spi_p->sem);
